namespace PlacementService {
	...
	void MouseRightClickToPlace(sf::Vector2i mousePosition) {
		...
		PlaceBlock(foundedBlock, MouseService::GetMousePosition(), true);
	}
	...
	bool PlaceBlock(Block* block, sf::Vector2i worldPosition, bool removeFromInventory) {
		if (removeFromInventory) {
			if (!InventoryService::FindItemById(block->GetID())) return false;
		}

		bool status = MapService::AddBlock(block, worldPosition);
